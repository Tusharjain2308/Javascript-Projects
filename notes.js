...
//wherease the false is default value and when true it means event capturing will take place
//also there is a e.preventDefault() functionality via which we can stop the default behaviour of any function or element

//59. Async Code
// there r 2 types of code in JS - 1. Blocking Code where we block the flow of program and files r read in sync behaviour
// 2. Non Blocking Code where the code doesn't block the flow of program and files can be read in asynchronous manner
...
//then is used to connect to resolve directly and then contains the values returned by resolve and below is an example how we can conmect resolve and then
...
//chaining of then and catch statments
...
//     console.log(error);
//   })
//   .finally(() => console.log("The promise is either rejectd or resolved"));

//finally always works !!

//using async await instead of then and catch -- both works the same way
